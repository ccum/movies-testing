...
public class MovieRepositoryJdbc implements MovieRepository {
	...
	public void saveOrUpdate(Movie movie) {
		jdbcTemplate.update("insert into movies (name, minutes, genre, director) values (?, ?, ?, ?)",
                movie.getName(), movie.getMinutes(), movie.getGenre().toString(), movie.getDirector().toString());
	...
	public Collection<Movie> findByDirector(String director) {
		...
		return jdbcTemplate.query("select * from movies where director = ?",args, movieMapper);
	}
